<?php	
	...
	$random = rand();
	$archivo = 'reporte60a90dias'.$random;
	...
	num_totalarticulosropa,num_totalarticulosmkp,num_importeropa,num_importemuebles,imp_marketplace,num_engancheropa,des_articulo,num_enganchemuebles,num_folioropa,num_foliomuebles,num_facturamkp,fec_movimiento,num_pagorecibido 
	FROM fn_rptplataforma3()";
	...
	$totalArchivos = "1";
		if($resultadoConsulta == false){
			...
			$rsEncabezado ='#registro'.'|'.'tipopago'.'|'.'nom_estatus'.'|'.'importe_total'.'|'.'importe_descuento'.'|'.'importe_empleado'.'|'.'idu_pedido'.'|'.'nom_email'.'|'.'num_clientecoppel'.'|'.'num_empleadocoppel'.'|'.
						...
						'num_totalarticulosropa'.'|'.'num_totalarticulosmkp'.'|'.'num_importeropa'.'|'.'num_importemuebles'.'|'.'imp_marketplace'.'|'.'num_engancheropa'.'|'.'des_articulo'.'|'.'num_enganchemuebles'.'|'.'num_folioropa'.'|'.'num_foliomuebles'.'|'.
		...
		foreach($resultadoConsulta as $row){
			...
			$rsEgistro =$icount.'|'.$row['tipopago'].'|'.$row['nom_estatus'].'|'.$row['importe_total'].'|'.$row['importe_descuento'].'|'.$row['importe_empleado'].'|'.$row['idu_pedido'].'|'.$row['nom_email'].'|'.$row['num_clientecoppel'].'|'.$row['num_empleadocoppel'].'|'.
						...
						$row['num_totalarticulosropa'].'|'.$row['num_totalarticulosmkp'].'|'.$row['num_importeropa'].'|'.$row['num_importemuebles'].'|'.$row['imp_marketplace'].'|'.$row['num_engancheropa'].'|'.$row['des_articulo'].'|'.$row['num_enganchemuebles'].'|'.$row['num_folioropa'].'|'.$row['num_foliomuebles'].'|'.
			...
			if($crearArchivo == $limiteRegistros){ 
				...
				echo'<a  lt="Descargar Reporte csv" style="text-decoration: none;color: #2196f3" href="./'.$archivo.$totalArchivos.'.csv" download><img src="download-icon.png"  height="16" width="16"> Descargar archivo 60 a 90 días</a></div>';
